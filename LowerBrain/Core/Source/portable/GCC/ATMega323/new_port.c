...
{
    //Addresses of the program counter are 32 Bit long in the ATXmega
	unsigned long usAddress;
	...
	*pxTopOfStack = 0x33;
	...
	//The jump addresses seem to have some very bad placement. so next "hack" is neccesarily
	usAddress <<= 8;
    usAddress &= 0xffffff00;
...
/*
    //Select the period time
	TCC0.PER = ulCompareMatch;
    //select the clock source and div by 64
	TCC0.CTRLA = (TCC0.CTRLA & ~TC0_CLKSEL_gm) | TC_CLKSEL_DIV64_gc;
	//single slope mode
	TCC0.CTRLB = (TCC0.CTRLB & ~TC0_WGMODE_gm) | TC_WGMODE_SS_gc; //TC_WGMODE_NORMAL_gc;
    //set int enable
...
ISR(BADISR_vect)
{
   // PORTE.OUT = 0xAA;
	...
	//void TCC0_OVF_vect( void ) __attribute__ ( ( signal, nacked ) );
